...
export async function runTranslationImport() {
  ...
  // 공격 주문 표는 이름이 전부 주문(Magic Missile, Fireball, Harm 등)이라
  // spellMap만으로 번역한다.
  const spellDamageMoves = game.settings.get(MODULE_ID, SETTINGS.SPELL_DAMAGE_MOVES);
  await game.settings.set(MODULE_ID, SETTINGS.SPELL_DAMAGE_MOVES, translateRows(spellMap, spellDamageMoves, stats));

  const upperHandMoveNames = game.settings.get(MODULE_ID, SETTINGS.UPPER_HAND_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.UPPER_HAND_MOVE_NAMES,
  ...
  const herculeanAppetitesMoveNames = game.settings.get(MODULE_ID, SETTINGS.HERCULEAN_APPETITES_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.HERCULEAN_APPETITES_MOVE_NAMES,
  ...
  const metalHurlantMoveNames = game.settings.get(MODULE_ID, SETTINGS.METAL_HURLANT_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.METAL_HURLANT_MOVE_NAMES,
    translateCommaList(moveMap, metalHurlantMoveNames, stats)
  );

  const eldritchTonesMoveNames = game.settings.get(MODULE_ID, SETTINGS.ELDRITCH_TONES_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.ELDRITCH_TONES_MOVE_NAMES,
    translateCommaList(moveMap, eldritchTonesMoveNames, stats)
  );

  const healingSongMoveNames = game.settings.get(MODULE_ID, SETTINGS.HEALING_SONG_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.HEALING_SONG_MOVE_NAMES,
    translateCommaList(moveMap, healingSongMoveNames, stats)
  );

  const viciousCacophonyMoveNames = game.settings.get(MODULE_ID, SETTINGS.VICIOUS_CACOPHONY_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.VICIOUS_CACOPHONY_MOVE_NAMES,
    translateCommaList(moveMap, viciousCacophonyMoveNames, stats)
  );

  const duelistParryMoveNames = game.settings.get(MODULE_ID, SETTINGS.DUELIST_PARRY_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.DUELIST_PARRY_MOVE_NAMES,
    translateCommaList(moveMap, duelistParryMoveNames, stats)
  );

  const bamboozleMoveNames = game.settings.get(MODULE_ID, SETTINGS.BAMBOOZLE_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.BAMBOOZLE_MOVE_NAMES,
  ...
  const signatureWeaponMoveNames = game.settings.get(MODULE_ID, SETTINGS.SIGNATURE_WEAPON_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.SIGNATURE_WEAPON_MOVE_NAMES,
    translateCommaList(moveMap, signatureWeaponMoveNames, stats)
  );

  const ironHideMoveNames = game.settings.get(MODULE_ID, SETTINGS.IRON_HIDE_MOVE_NAMES);
  await game.settings.set(MODULE_ID, SETTINGS.IRON_HIDE_MOVE_NAMES, translateCommaList(moveMap, ironHideMoveNames, stats));

  const mercilessMoveNames = game.settings.get(MODULE_ID, SETTINGS.MERCILESS_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.MERCILESS_MOVE_NAMES,
    translateCommaList(moveMap, mercilessMoveNames, stats)
  );

  const interrogatorMoveNames = game.settings.get(MODULE_ID, SETTINGS.INTERROGATOR_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.INTERROGATOR_MOVE_NAMES,
    translateCommaList(moveMap, interrogatorMoveNames, stats)
  );
  ...
  const improvedWeaponMoveNames = game.settings.get(MODULE_ID, SETTINGS.IMPROVED_WEAPON_MOVE_NAMES);
  await game.settings.set(
    MODULE_ID,
    SETTINGS.IMPROVED_WEAPON_MOVE_NAMES,

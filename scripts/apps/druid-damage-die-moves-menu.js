...
function normalizeRow(raw) {
  return {
    name: (raw?.name ?? "").trim(),
    dieSize: Math.max(1, parseInt(raw?.dieSize, 10) || 0)
  };
}

export class DruidDamageDieMovesMenu extends FormApplication {
  constructor(...args) {
    super(...args);
    this.rows = foundry.utils.deepClone(game.settings.get(MODULE_ID, SETTINGS.DRUID_DAMAGE_DIE_MOVES));
  }

  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: "dwauto-druid-damage-die-moves",
      title: game.i18n.localize("DWAUTO.DruidDamageDieMoves.Title"),
      template: `modules/${MODULE_ID}/templates/druid-damage-die-moves-settings.html`,
      width: 560,
      closeOnSubmit: true
    });
  }
  ...
  getData() {
    return {
      hint: game.i18n.localize("DWAUTO.DruidDamageDieMoves.Hint"),
      rows: this.rows
    };
  }
  ...
  async _updateObject(event, formData) {
    ...
    await game.settings.set(MODULE_ID, SETTINGS.DRUID_DAMAGE_DIE_MOVES, rows.filter((r) => r.name));
  }
}

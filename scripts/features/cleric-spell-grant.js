...
// Favor)/레인저 황무지의 신(God Amidst The Wastes)으로 예배를 얻은
// 경우는 그 클레릭 레벨(features/note-moves.js가 관리, features/
// spell-preparation.js의 예배 준비 한도 계산과 동일한 기준)을 쓴다 —
// 셋 다 "그 레벨 이하의 클레릭 기본 주문 컴펜디엄 전체"를 대상으로,
// 이미 갖고 있는 주문(이름 기준, 스펠북 증보 등으로 이미 받은 것 포함)은
// 제외하고 나머지를 한 번에 스펠북에 추가한다(system.prepared는 false로
// 시작 — 실제로 쓰려면 예배/주문 준비를 따로 해야 한다).
...
function getEffectiveClericLevel(actor) {
  const override = getClericLevelOverride(actor);
